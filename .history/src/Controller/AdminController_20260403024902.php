...
class AdminController extends AbstractController
{
    ...
    {
        $userCount = $entityManager->getRepository(Personne::class)->count([]);
        ...
        $latestUsers = $entityManager->getRepository(Personne::class)->findBy([], ['id' => 'DESC'], 5);
    ...
    public function changeRole(Request $request, Personne $user, EntityManagerInterface $entityManager): Response
    {
        ...
        if (in_array($newRole, ['ROLE_USER', 'ROLE_ADMIN'])) {
             // In Personne entity role could be string or array. Wait, in Govibe it's array typically in Symfony or maybe string.
             // Usually Symfony UserInterface expects an array of roles. Let's look at what we've seen earlier. RegistrationFormType uses 'user'/'admin' directly string if 'expanded' is false? Wait, symfony `getRoles()` returns array.
             // Actually, `Personne.php` might have a `setRole(string $role)` or `setRoles(array $roles)`.
             // I'll assume setRole takes a string because the property in RegistrationFormType was a string 'user'/'admin'. Wait! No, `getRoles()` is the standard... Let's just use what works or fetch what it is.
             // If setRole takes a string:
